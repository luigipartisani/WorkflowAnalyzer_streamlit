...
{
    public class WorkflowCPB : Workflow
    {
        ...
        public WorkflowCPB(string key, string title, Action<StateContext> drawPage) : base(key, title)
        {
            _DrawPage = drawPage;

            List<string> activities = GetActivities(typeof(WorkflowCPB));
        ...
        private void _AddActivity_Modulo(Workflow wf)
        {
            ...
            a.StaticInput = new Input(InputType.Single, new List<InputItem>(new InputItem[] {
                 new InputItem("CPB.DESK","CPB.DESK - Concordato Preventivo Biennale", "CPB.DESK", true),
            }));
