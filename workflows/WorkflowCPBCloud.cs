...
{
    public class WorkflowCPBCloud : Workflow
    {
        ...
        public WorkflowCPBCloud(string key, string title, Action<StateContext> drawPage) : base(key, title)
        {
            _DrawPage = drawPage;

            List<string> activities = GetActivities(typeof(WorkflowCPBCloud));
        ...
        private void _AddActivity_Modulo(Workflow wf)
        {
            ...
            a.StaticInput = new Input(InputType.Single, new List<InputItem>(new InputItem[] {
                 new InputItem("CPB.SAAS","CPB.SAAS - Concordato Preventivo Biennale - cloud", "CPB.SAAS", true),
            }));
