...
{
    public class WorkflowPDSInnovazioneSoftware : Workflow
    {
        ...
        public WorkflowPDSInnovazioneSoftware(string key, string title, Action<StateContext> drawPage) : base(key, title)
        {
            _DrawPage = drawPage;

            List<string> methods = ShowMethods(typeof(WorkflowPDSInnovazioneSoftware));
        ...
        private void _AddActivity_TipoCliente(Workflow wf)
        {
            ...
            a.StaticInput = new Input(InputType.Single, new List<InputItem>(new InputItem[] {
                new InputItem("COMM", "Commercialista"),
                new InputItem("AZI", "Azienda")
            }));
            a.DrawPage = _DrawPage;

            Branch b1 = a.CreateBranchTo("datiSHS");
        }
