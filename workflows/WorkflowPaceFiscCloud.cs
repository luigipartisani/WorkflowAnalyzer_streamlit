...
{
	public class WorkflowPaceFiscCloud : Workflow
	{
		...
		public WorkflowPaceFiscCloud(string key, string title, Action<StateContext> drawPage) : base(key, title)
		{
			_DrawPage = drawPage;

			List<string> methods = ShowMethods(typeof(WorkflowPaceFiscCloud));
		...
		private void _AddActivity_TipoGestioneNuovo(Workflow wf)
		{
			...
			a.StaticInput = new Input(InputType.Single, new List<InputItem>(new InputItem[] {
				new InputItem("6568109", "6568109 - Pace Fiscale 2023","6568109",true)
			}));
