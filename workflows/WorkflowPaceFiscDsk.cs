...
{
	public class WorkflowPaceFiscDsk : Workflow
	{
		...
		public WorkflowPaceFiscDsk(string key, string title, Action<StateContext> drawPage) : base(key, title)
		{
			_DrawPage = drawPage;

			List<string> methods = ShowMethods(typeof(WorkflowPaceFiscDsk));
		...
		private void _AddActivity_TipoGestioneNuovo(Workflow wf)
		{
			...
			a.StaticInput = new Input(InputType.Single, new List<InputItem>(new InputItem[] {
				new InputItem("6568103", "6568103 - Pace Fiscale 2023","6568103",true)
			}));
