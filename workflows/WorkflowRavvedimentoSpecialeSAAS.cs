...
{
	public class WorkflowRavvedimentoSpecialeSAAS : Workflow
	{
		...
		public WorkflowRavvedimentoSpecialeSAAS(string key, string title, Action<StateContext> drawPage) : base(key, title)
		{
			_DrawPage = drawPage;

			List<string> activities = GetActivities(typeof(WorkflowRavvedimentoSpecialeSAAS));
		...
		private void _AddActivity_Modulo(Workflow wf)
		{
			...
			a.StaticInput = new Input(InputType.Single, new List<InputItem>(new InputItem[] {
				 new InputItem("RS.SAAS","RS.SAAS - Ravvedimento Speciale: Gestione", "RS.SAAS", true),
			}));
